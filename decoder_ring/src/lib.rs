...
fn get_counts() -> HashMap<char, f32> {
...
fn stats_analysis(text: &str) -> Vec<(char, u32, f32, Option<f32>, f32)> {
    ...
    let eng_freq_map = get_counts();
    let _emg_freq_map: HashMap<char, f32> = eng_freq_map.iter().map(|(k, v)| (*k, *v)).collect();

    let mut results = Vec::new();
    for (letter, count) in &counts {
        let freq = *count as f32 / total as f32;
        let eng_freq = eng_freq_map.get(&letter.to_ascii_lowercase()).cloned();
        let eng_fre_diff = eng_freq.map_or(0.0, |f| (freq - f).abs());

        results.push((*letter, *count, freq, eng_freq, eng_fre_diff));
    }
...
pub fn guess_shift(text: &str, depth: u8) -> (u8, u8, String, f32) {
    ...
    for shift in 0..depth {
        ...
        let mut score = 0.0;
        for (_, _, _freq, eng_freq, eng_freq_diff) in stats {
            if let Some(eng_freq) = eng_freq {
                score += 1.0 + eng_freq_diff / eng_freq;
            }
        }
